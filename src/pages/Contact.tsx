...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (!FORM_ENDPOINT) { setStatus('error'); return }
          ...
          <AnimateIn type="fade-right">
            <div className="contact-form-wrap">
              ...
              <form className="contact-form" onSubmit={handleSubmit}>
                <label>First name *<input type="text" name="firstName" required placeholder="Your first name" /></label>
                <label>Last name *<input type="text" name="lastName" required placeholder="Your last name" /></label>
                <label>Email *<input type="email" name="email" required placeholder="you@example.com" /></label>
                <label>Phone<input type="tel" name="phone" placeholder="Optional" /></label>
                <label>Message *<textarea name="message" rows={5} required placeholder="Leave us a message..." /></label>
          ...
          <AnimateIn type="fade-left" delay={100}>
            <div className="contact-details">
              ...
              <ul className="contact-list">
                {data.contacts.map((c) => (
                  <li key={c.role}>
                    <strong>{c.role}</strong> &ndash; {c.name}<br />
                    <a href={`mailto:${c.email}`}>{c.email}</a>
                  </li>
                ))}
              </ul>
              <h2>Visit us</h2>
              <p>{data.venue.name}<br />{data.venue.street}<br />{data.venue.suburb}</p>
              <h2>Postal address</h2>
              <p>{data.postal.line1}<br />{data.postal.line2}</p>
            </div>
          </AnimateIn>
        </div>
      </section>
